...
public:
    ...
    void zip(ListLinkedDouble& other) {
        Node* currXs = head->next;
        Node* antZs = other.head;
        Node* currZs = other.head->next;

        while (currXs != head && currZs != other.head) {
            currZs->prev = currXs;
            antZs->next = currZs->next;
            currZs->next = currXs->next;
            currXs->next->prev = currZs;
            currXs->next = currZs;
        ...
        if (currZs != other.head) {
            currXs->prev->next = currZs;
            currZs->prev = currXs->prev;
            antZs->prev->next = currXs;
            currXs->prev = antZs->prev;
            antZs->next = antZs;
        }
