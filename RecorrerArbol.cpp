...
//Coste de la funcion suponiendo que los arboles estan equilibrados
// T(n) = c0*log(n) si n = 1
// T(n) = T(ni) + T(nd) + c1*log(n) si n > 1
int recorrer_arbol(const BinTree<string> &arbol, const MapTree<string,int> &aux) {
    if (arbol.right().empty() && arbol.left().empty()) {
        if (aux.contains(arbol.root()))
            return aux.at(arbol.root());
        else 
            return stoi(arbol.root());
    }
    else {
        int izq = recorrer_arbol(arbol.left(),aux);
        int der = recorrer_arbol(arbol.right(),aux);
...
//Coste de la funcion
//N: Numero de varibles
//M: Procesamiento de un unico nodo dentro de la funcion auxiliar
//S: Maximo numero de nodos en un arbol ast 
...
bool tratar_caso() {
    ...
    MapTree<string,int> letras;

    //Numero de lineas O(N)
    for (int i = 0; i < n; i++) {
        //El parseo de cada linea es O(N)
        BinTree<string> a = parse(cin);

        //Coste de la funcion para calcular el valor de la variable es O(S log M)
        int s = recorrer_arbol(a.right(), letras);

        //Suponiendo que son arboles reequlibrados el coste es O(log N)
        letras[a.left().root()] = s;
    }

    for (MapTree<string,int> :: MapEntry &e: letras) {
