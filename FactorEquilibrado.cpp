...
//O(n)
pair<bool, int> equilibrado(const BinTree<int>& t) {
    if (t.empty()) {
        return{ true,0 };
    }
        ...
        auto [bl, l] = equilibrado(t.left());
        auto [br, r] = equilibrado(t.right());
...
bool factor_equilibrado(const BinTree<int>& t) {
    ...
    return get<0>(equilibrado(t));
}

// Función para tratar un caso de prueba
void tratar_caso() {
    BinTree<int> t = read_tree<int>(cin);
    if (factor_equilibrado(t)) {
        cout << "SI" << endl;
    }
    else {
        cout << "NO" << endl;
